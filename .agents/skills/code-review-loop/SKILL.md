...
Drive the branch or session-derived review target to a clean functional result. Use `code-review` for every review pass so target selection, branch/PR creation when needed, reviewer coverage, severity, and chat-only reporting stay consistent.
...
2. Start a fresh five-lens reviewer cohort through `code-review` against the complete selected target. It may use the current PR diff, create a feature branch and draft PR for intended local changes, or review verified session commits already pushed to the default branch.
...
4. Record the round's reviewer identities, lens coverage receipts, Rules coverage ledger, baseline head SHA, findings, and dismissals. If any receipt is missing, failed, duplicated, incomplete, or stale, rerun the affected lens and do not update `noFunctionalFindingRounds` until complete coverage exists.
5. Update `noFunctionalFindingRounds`: reset it to `0` if this fully covered round has a functional finding; otherwise increment it, including when the round is clean. Apply the smallest complete fixes while preserving the requested behavior and unrelated worktree changes.
6. Stage only the intended task changes and review fixes, commit them, and push the current PR branch. If the initial target was session commits already on the default branch and fixes are needed, first create a new branch from the current default-branch head, then push it and create a draft PR for the fixes.
7. Run `code-review` again on the complete updated target with a fresh reviewer cohort, supplying the dismissal ledger without suppressing newly applicable rule findings.
8. Repeat steps 3–7 until either:
...
- the number of review passes and the completed reviewer lenses for each counted round;
...
- confirmation that the final selected-target review returned no findings with complete reviewer coverage, plus any degraded-mode reason.
